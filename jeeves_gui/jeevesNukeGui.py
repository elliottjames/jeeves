...
class NukeConnectGui(QWidget):
    def __init__(self, parent=None):
        ...
        self.open_my_button = QPushButton('&Refresh')
        self.open_my_button.setMinimumSize(QSize(245, 0))
        self.open_my_button.setMaximumSize(QSize(245, 100))
        self.open_my_button.clicked.connect(self.refreshScripts)
        
        self.hbox7_refresh = QHBoxLayout()
        self.hbox7_refresh.addWidget(self.open_my_button)
        ...
        self.vbox.addLayout(self.hbox7_refresh)
    ...
    def addshot(self):
        ...
        text, ok = QInputDialog.getText(self, 'Add Shot', '')

        if not ok:
            return
        
        if text.startswith('sh_'):
            text = text.split('sh_')[1]
        newshot = 'sh_' + text.replace(' ', '_')
        #print newshot
        
        if jeeves_core.searchJob.searchForShot(self.job, newshot):
            jeeves_core.make.MakeShotCmds(self.job, newshot)
        else:
            print 'shot exists'
